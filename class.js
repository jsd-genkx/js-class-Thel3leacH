...
class Product {
    ...
    constructor (name, price, manufacturedDate) {
      Product.validateName(name); // call error
      this.name = name;
      this.#price = price;
    ...
    set price(value) {
      if (value < 0) throw new ValidationError("Validate fail: price shouldn't negative");
      this.#price = value;
    }
    ...
    getAge() {
      const daysOld = Date.now() - this.manufacturedDate.getTime();
      return Math.floor(daysOld/(1000 * 60 * 60 * 24));
    }

    static validateName(name) {
      if (!name || name.length < 2) {
        throw new ValidationError("name is empty or shorter than 2 characters")
      }
    }
}
